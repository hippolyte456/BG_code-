...
def creation_model(time = 1000.0 , stimulus = False, modelID = 1, NbChannels = 2 ):
    ...
    if stimulus :
        proj_stimulus = Projection(pre=CSN[1], post=MSN[1], target='exc', synapse=Basic) 
        proj_stimulus.connect_one_to_one(weights= 2*W_CSN_MSN , delays = latency['dCSN_MSN'])
    ##########################################################################################################################
    ...
    plt.savefig('log/test_model/firingrate_model%d.png'%modelID)
    plt.show()
...
########################## valeur de Shapley #################################
#mettre le modele sur 2 channels 
#monitoré GPi firing rate 

#faire varier au cours de l'experience la salience des inputs des 2 channels (possible car pas d'hysteresis)

#calculate the distorsion/ efficiency pour chaque couple de salience
...
input_CSN = Neuron( 
            parameters = """
                ...
                s2 = 20               
            """,
            equations = """ r = paliers(s1,s2) """,
            functions = """ paliers(x1,x2) = if 600.0 > 500.0 : x2 else: x1 """)
            #functions = salience[0][0] puis salience[1][0] puis... salience[15][0] 



#CSNinput_level = [2,8,14,20]
        #PTNinput_level = [15,25,35,46]
        #CSNsaliences = Constant('CSNsaliences', [(2,2) , (2,8) , (2,14) , (2,20) , (8,8) , (8,14) , (8,20) , (14,14) , (14,20) , (20,20)])
        #PTNsaliences = [(15,15) , (15,25) , (15,35) , (15,46) , (25,25) , (25,35) , (25,46) , (35,35) , (35,46) , (46,46)]
        #time_step = 1000.0
        #inp = TimedArray(rates = [10,5], period = 200.0)
        #10**(1/3) on prend la multiplication ou l'addition pour faire les ecarts ?





#exemple utilisation de subplots
