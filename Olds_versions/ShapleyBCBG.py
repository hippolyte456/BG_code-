...
#regarder le comportmeent des selections lorsqu'il y a 2 canaux / lorsqu'il y a 6 canux --> peut-on réduire à 2 canaux sans soucis ? 
...
NbChannels = 2 ### attention si plus de 9 à l'affichage...
...
def creation_model(time = 10000.0):
    clear()
    ...
    ### input ###    
    CSN = Population(name='CSN', geometry=1, neuron = input_CSN )
    PTN = Population(name='PTN', geometry=1, neuron = input_PTN )
    ...
    proj_CSN_MSN = Projection(pre=CSN, post=MSN[0], target='exc', synapse=Basic) 
    proj_CSN_FSI = Projection(pre=CSN, post=FSI[0], target='exc', synapse=Basic)
    proj_PTN_MSN = Projection(pre=PTN, post=MSN[0], target='exc', synapse=Basic)
    proj_PTN_FSI = Projection(pre=PTN, post=FSI[0], target='exc', synapse=Basic)
    proj_PTN_STN = Projection(pre=PTN, post=STN[0], target='exc', synapse=Basic)

    proj_CSN_MSN_stim = Projection(pre=CSNstim, post=MSN[1], target='exc', synapse=Basic) 
    proj_CSN_FSI_stim = Projection(pre=CSNstim, post=FSI[1], target='exc', synapse=Basic)
    proj_PTN_MSN_stim = Projection(pre=PTNstim, post=MSN[1], target='exc', synapse=Basic)
    proj_PTN_FSI_stim = Projection(pre=PTNstim, post=FSI[1], target='exc', synapse=Basic)
    proj_PTN_STN_stim = Projection(pre=PTNstim, post=STN[1], target='exc', synapse=Basic)
    ...
    proj_CSN_MSN_stim.connect_one_to_one(weights= W_CSN_MSN , delays = latency['dCSN_MSN'])
    proj_CSN_FSI_stim.connect_one_to_one(weights= W_CSN_FSI , delays = latency['dCSN_FSI'])
    proj_PTN_MSN_stim.connect_one_to_one(weights= W_PTN_MSN , delays = latency['dPTN_MSN'])
    proj_PTN_FSI_stim.connect_one_to_one(weights= W_PTN_FSI , delays = latency['dPTN_FSI'])
    proj_PTN_STN_stim.connect_one_to_one(weights= W_PTN_STN , delays = latency['dPTN_STN'])
    ...
    axs[4,0].plot(dt()*np.arange(time), r) 
    rstim = GPiSNrmonitors['mono2'].get('r')
    axs[4,1].plot(dt()*np.arange(time), rstim)
...
a = creation_model(time = 10000.)





def calcul_eff_dist(firing_rate,firing_rate_stim):
    subarray1 = firing_rate[250:750]
    subarray2 = firing_rate[1250:1750]
    subarray3 = firing_rate[2250:2750]
    subarray4 = firing_rate[3250:3750]
    subarray5 = firing_rate[4250:4750]
    subarray6 = firing_rate[5250:5750]
    subarray7 = firing_rate[6250:6750]
    subarray8 = firing_rate[7250:7750]
    subarray9 = firing_rate[8250:8750]
    subarray10 = firing_rate[9250:9750]

    subarray1_stim = firing_rate_stim[250:750]
    subarray2_stim = firing_rate_stim[1250:1750]
    subarray3_stim = firing_rate_stim[2250:2750]
    subarray4_stim = firing_rate_stim[3250:3750]
    subarray5_stim = firing_rate_stim[4250:4750]
    subarray6_stim = firing_rate_stim[5250:5750]
    subarray7_stim = firing_rate_stim[6250:6750]
    subarray8_stim = firing_rate_stim[7250:7750]
    subarray9_stim = firing_rate_stim[8250:8750]
    subarray10_stim = firing_rate_stim[9250:9750]

    subarrays = [subarray1 , subarray2 , subarray3 , subarray4 , subarray5 , subarray6 , subarray7 , subarray8 , subarray9 , subarray10]
    subarrays_stim = [subarray1_stim , subarray2_stim , subarray3_stim , subarray4_stim , subarray5_stim , subarray6_stim , subarray7_stim , subarray8_stim , subarray9_stim , subarray10_stim]
    rGPi_rest = mean(subarray1)
    
    efficacity, distortion = [],[]
    for i in range(10):
        rGPi = mean(subarrays[i])
        rGPi_stim = mean(subarrays_stim[i])
        eff1 = max(0, 1 - rGPi / rGPi_rest)
        eff2 = max(0, 1 - rGPi_stim / rGPi_rest)
        eff = max( eff1, eff2)
        dist = abs( eff1 - eff2 ) / (eff+0.00000001)
...
#attribuer sa valeur a chacune des projections gardees 

#faire cela pour toutes les lésions possibles
# calculer la valeur de Shapley pour chacune des projections  

#faire cette analyse pour les 13 modeles ?
...
#saliences = [(0,0) , (0,1) , (0,2) , (0,3) , (0,4) , (1,1) , (1,2) , (1,3) , (1,4) , (2,2) , (2,3) , (2,4) , (3,3) , (3,4) , (4,4)] #cb on test de salience differentes ? je pense que 3 ou 4 ça suffit... de toute manière le pattern est complétement régulier : si je le fais avec 4 je suis capable de predire avec 11 !!!

#for salience1, salience2 in saliences : 
#    eff1 = 1 - r(GPiSNr)sachantsaliencechannels1,2 / r(GPiSNr)sachantsalience=0 (pour channel 1)
#    eff2 = 1 - r(GPiSNr)sachantsaliencechannels1,2 / r(GPiSNr)sachantsalience=0 (pour channel 2)
#    eff = max( eff1, eff2)
#    distortion = abs( eff1 - eff2 ) / eff
#
#selection_value_eff = mean(eff)
#selection_value_distortion = 1 - mean(distortion)
#tot_selection_value = mean(np.array(selection_value_distortion,selection_value_eff))   #à normaliser
